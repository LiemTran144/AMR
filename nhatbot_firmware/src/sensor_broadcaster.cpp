...
controller_interface::CallbackReturn SensorBroadcaster::on_init()
{
  try
  {
    battery_pub_ = get_node()->create_publisher<sensor_msgs::msg::BatteryState>("~/battery_state", 10);  // get_node() từ ControllerInterface
    temp_pub_    = get_node()->create_publisher<std_msgs::msg::Float32>("~/motor_temp", 10);             // topic ~/motor_temp day du la nhatbot_sensors/motor_temp
