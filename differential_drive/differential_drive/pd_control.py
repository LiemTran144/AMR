...
#         print(f"PD Motion Planner parametersssss: kp={self.kp}, kd={self.kd}, step_size={self.step_size}, max_linear_velocity={self.max_linear_velocity}, max_angular_velocity={self.max_angular_velocity}, deathband={self.deathband}")

#         # Subscribers and publishers
#         # self.path_sub = self.create_subscription(Path, '/liem/ui_path', self.path_callback, 10)
#         self.path_sub = self.create_subscription(Path, '/plan', self.path_callback, 10)
#         self.cmd_pub = self.create_publisher(Twist, "/nav/cmd_vel", 10)    #"/liem_controller/cmd_vel" "/UI/cmd_vel" 
...
#         if not self.transform_plan(robot_pose_transform.header.frame_id):  #id= target_frame = 'odom'
...
class PDMotionPlanner(Node):
    ...
    def control_loop(self):
        ...
        self.get_logger().info(f"Cmd Vel => Linear: {cmd_vel.linear.x:.2f}, Angular: {cmd_vel.angular.z:.2f}")
