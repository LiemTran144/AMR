...
#include "liem_controller/pd_motion_planner.hpp"
...
{
PDMotionPlanner::PDMotionPlanner() : Node("pd_motion_planner_node"), kp_(1.5), kd_(0.1), step_size_(0.2), max_linear_velocity_(0.1),
                                                        max_angular_velocity_(0.2), prev_angular_error_(0.0), prev_linear_error_(0.0)

{
  ...
  path_sub_ = create_subscription<nav_msgs::msg::Path>(
      "/plan", 10, std::bind(&PDMotionPlanner::pathCallback, this, std::placeholders::_1));
        
  cmd_pub_ = create_publisher<geometry_msgs::msg::Twist>("/nav/cmd_vel", 10);
...
void PDMotionPlanner::controlLoop()
{
  ...
  if(distance <= 0.1)
  {
    ...
    RCLCPP_INFO(get_logger(), "Last error distances: linear %.3f m, angular %.3f m", dx, dy);
  ...
  RCLCPP_INFO(get_logger(), "Next pose in robot frame: (%.2f, %.2f)", 
    next_pose_robot_tf.getOrigin().getX(), next_pose_robot_tf.getOrigin().getY());
...
bool PDMotionPlanner::transformPlan(const std::string & frame)
{
  ...
  try{
    transform = tf_buffer_->lookupTransform(frame, global_plan_.header.frame_id, tf2::TimePointZero); // transform from plan frame to robot frame
...
int main(int argc, char *argv[])
{
  ...
  auto node = std::make_shared<nhatbot_controller::PDMotionPlanner>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}